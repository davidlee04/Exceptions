...
class ExceptionsTest {
	...
	public void testDivideByZeroException() {
		try {
			assertEquals(1.25, em.divide(5.0, 4.0));
			assertEquals(30, em.divide(90, 3));
			em.divide(9, 0.0	);
			fail("illegalargumentexception");
		} catch (IllegalArgumentException e) {
			e.printStackTrace();
			assertEquals(1.0, em.divide(5.0, 5.0));
		}
	...
	public void testReverseString() {
		try {
			assertEquals("on", em.reverseString("no"));
			assertEquals("hello", em.reverseString("olleh"));
			em.reverseString("");
			fail("illegalstateexception");
		} catch (IllegalStateException e) {
			e.printStackTrace();
			assertEquals("hi", em.reverseString("ih"));
		}
